...
async function loadManifest() {
  // Fast path: use the combined manifest generated by the R2 importers.
  // This avoids a full R2 scan before the reader can render.
...
function getDefaultFitMode() {
  // Default to full-height on desktop and mobile before fullscreen.
  // Mobile fullscreen portrait still switches to width-fit inside applyFullscreenState.
  return 'page';
}


function setUiHidden(hidden) {
  // Outside explicit fullscreen, never hide the toolbar automatically.
  const nextHidden = state.immersiveFullscreen ? Boolean(hidden) : false;
  state.uiHidden = nextHidden;
  document.body.classList.toggle('reader-ui-hidden', nextHidden);
}
...
function renderAll() {
  if (!state.immersiveFullscreen) setUiHidden(false);
